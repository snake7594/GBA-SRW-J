...
JSON_PATH = 'battle_dialogue.json'
...
def main():
    ...
    r = ins.build(out_rom)
    print(f"재배치 블록: {r['changed']}개, 번역 토큰: {r['tokens']}개")
    print(f"합체기 블록(blk193): 원위치 길이보존 {r['len_tokens']}토큰 (도몬 버그 방지)")
    print(f"확장영역 0x{r['dest']:X}부터 0x{r['used']:X} 사용, ROM 0x{r['rom']:X}")
    print(f"\n완료! 저장됨: {out_rom}")
    print("mGBA 등 에뮬레이터나 실기에서 전투 대사·합체기가 한글로 나오는지 확인하세요.")
