...
def verify(rom_path, json_path):
    ...
    # [A] 추출 정확성 — off/ptrs 위치의 ROM 바이트 == enc(jp), 길이 == len
    a_ok = a_bad = a_occ = 0
    a_samples = []
    for e in ents:
        jp = e['jp']; ln = e['len']
    ...
    untranslated = [e for e in ents
                    if not e['ko'] and e['jp'].strip('！\n\u3000・') and jp_residual(e['jp'])]
    translated = [e for e in ents if e['ko'] and not jp_residual(e['ko'])]
    nondlg = [e for e in ents if not e['jp'].strip('！\n\u3000・')]
    print(f"\n[D] 번역 상태   : 번역완료 {len(translated):,} / 미번역(대사·ko공란) "
          f"{len(untranslated)} / 미완성(카나잔존) {len(unfinished)} / 비대사 {len(nondlg)}")
    for e in unfinished[:5]:
        print(f"      ⚠ 카나잔존 {e['off']}  ko={e['ko'][:24]!r}")
    for e in untranslated[:5]:
        print(f"      ⚠ 미번역  {e['off']}  jp={e['jp'][:24]!r}")

    ok = (a_bad == 0 and not missing and not extra and c1 and c2 and not unfinished)
    print(f"\n{'═'*46}")
